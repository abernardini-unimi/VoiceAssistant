// Usage:
//
//	go run examples/web-voice-assistant-groq/main.go
...
func createPipeline(ctx context.Context, session *realtimeapi.Session, cfg PipelineConfig) (*pipeline.Pipeline, error) {
	...
	asrConfig := custom_elements.CustomWhisperConfig{
	...
	asrElem, err := custom_elements.NewCustomWhisperElement(asrConfig)
	if err != nil {
		return nil, err
	}
